...
This file was retrieved from https://gist.github.com/zed/92df922103ac9deb1a05 on 25 Jun 2019. Full credit to GitHub
user "zed".
"""
...
# FUNCTIONS ADDED BY JLL #
def tai93_to_utc(tai93_seconds):
    """Convert TAI93 time given as TAI seconds since 1 Jan 1993 to UTC time (as a datetime object)"""

    # Assuming that TAI93 time is that defined in https://newsroom.gsfc.nasa.gov/sdptoolkit/primer/time_notes.html#TAI
    # i.e. atomic time seconds since 00:00 1 Jan 1993, UTC, then adding the seconds to 1 Jan 1993 in TAI time, then
    # converting the result back to UTC *should* give the correct UTC time.
    td = timedelta(seconds=tai93_seconds)
    epoch = utc_to_tai(datetime(1993, 1, 1))
    tai_date = epoch + td
    return tai_to_utc(tai_date)
