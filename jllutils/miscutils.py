...
def flood_fill_threshold(data, start_pos, threshold, allow_diagonals=False):
    """
    Find all grid cells connected to a starting point with values above a threshold.

    This function uses a Breadth-First Search (BFS) flood-fill algorithm to 
    identify contiguous regions in a 2D array that exceed a specified threshold, 
    starting from a user-defined seed point.

    Note: function initially generated by Gemini.

    Parameters
    ----------
    data : ndarray
        A 2D NumPy array of numeric values to be analyzed.
    start_pos : tuple of int
        The starting (row, column) coordinates for the flood fill, 
        formatted as (row, col).
    threshold : float or int
        The value used to filter cells. Only cells where `data[row, col] > threshold` 
        and which are connected to the starting point will be marked True.
    allow_diagonals : bool, optional
        Determines the connectivity logic. If True, uses 8-way connectivity 
        (includes diagonals). If False, uses 4-way connectivity (up, down, 
        left, right). Default is False.

    Returns
    -------
    mask : ndarray of bool
        A boolean array of the same shape as `data`. Cells that are part 
        of the connected component above the threshold are set to True; 
    ...
    # Boundary check and initial threshold check
    if not (0 <= start_r < rows and 0 <= start_c < cols):
        return mask
    if data[start_r, start_c] <= threshold:
        warnings.warn(f'Starting point ({start_r}, {start_c}) is below the threshold')
        return mask
    ...
    while queue:
        ...
        for dr, dc in offsets:
            ...
            if 0 <= nr < rows and 0 <= nc < cols:
                # Check if not visited and meets threshold
                if not mask[nr, nc] and data[nr, nc] > threshold:
                    mask[nr, nc] = True
                    queue.append((nr, nc))

    return mask

def flood_fill_at_coords(data, x, y, x0, y0, threshold, allow_diagonals=False):
    if x.size == data.shape[0] + 1:
        x = 0.5 * (x[:-1] + x[1:])
    elif x.size != data.shape[0]:
        raise ValueError('x must either be the same length as the first dimension of data or one longer (if it represents edges)')

    if y.size == data.shape[1] + 1:
        y = 0.5 * (y[:-1] + y[1:])
    elif y.size != data.shape[1]:
        raise ValueError('y must either be the same length as the second dimension of data or one longer (if it represents edges)')
    dist = ((x[:, np.newaxis] - x0)**2 + (y[np.newaxis, :] - y0)**2)
    imin, jmin = np.unravel_index(np.argmin(dist), dist.shape)
    return flood_fill_threshold(data=data, start_pos=(imin, jmin), threshold=threshold, allow_diagonals=allow_diagonals)



def extract_polygon_from_mask(mask, X, Y):
    """
    Extract polygon coordinates that trace the boundary of a boolean mask.

    Note that this function was originally created by Gemini.

    Parameters
    ----------
    mask : ndarray of bool
        A 2D boolean array where True represents the area to be traced.
    X : ndarray
        1D array of length N+1 defining the horizontal edge coordinates.
    Y : ndarray
        1D array of length M+1 defining the vertical edge coordinates.

    Returns
    -------
    polygons : list of ndarray
        A list of polygons. Each polygon is an Nx2 array of (x, y) coordinates.
        Multiple polygons are returned if the mask contains holes or 
        disconnected regions.
    """
    try:
        from skimage import measure
    except ImportError:
        raise ImportError('scikit-image is required to use the extract_polygon_from_mask function')
    # 1. Find contours at the level of 0.5 (the boundary between False/0 and True/1)
    # find_contours returns coordinates in (row, col) format.
    # In your logic, row (dim 0) maps to X and col (dim 1) maps to Y.
    contours = measure.find_contours(mask.astype(float), 0.5)

    physical_polygons = []

    # 2. Map index-space coordinates to physical X and Y coordinates
    # In skimage, the center of pixel (i, j) is at index (i, j).
    # The boundary between pixel i and i-1 is at i - 0.5.
    # According to your setup, the boundary between cell i and i-1 is X[i].
    # Therefore, index i-0.5 maps to X[i].

    # Create an index reference for the edge arrays
    x_indices = np.arange(len(X)) - 0.5
    y_indices = np.arange(len(Y)) - 0.5

    for contour in contours:
        # Extract row/col indices from the contour
        res_i = contour[:, 0]
        res_j = contour[:, 1]

        # Interpolate to find the actual X and Y values
        # np.interp(target_indices, source_indices, source_values)
        poly_x = np.interp(res_i, x_indices, X)
        poly_y = np.interp(res_j, y_indices, Y)

        # Combine into an (N, 2) array
        polygon = np.column_stack((poly_x, poly_y))
        physical_polygons.append(polygon)

    return physical_polygons


def polygon_to_rect(poly, buffer=None):
    x1 = np.min(poly[:,0])
    x2 = np.max(poly[:,0])
    y1 = np.min(poly[:,1])
    y2 = np.max(poly[:,1])
